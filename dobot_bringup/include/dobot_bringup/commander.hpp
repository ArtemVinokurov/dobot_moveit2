...
public:
    ...
    explicit CR5Commander(const std::string& ip)
        : current_joint_{}, current_vel_joint_{}, tool_vector_{}, real_time_data_{}, is_running_(false)
    {
        is_running_ = false;
        motion_port_tcp_ = std::make_shared<TcpClient>(ip, 30003);
    ...
    ~CR5Commander()
    {
        is_running_ = false;
        thread_->join();
        RCLCPP_INFO(getLogger(), "Success join thread");
    }

    void getCurrentJointStatus(double* q, double* qd)
    {
        mutex_.lock();
        memcpy(q, current_joint_, sizeof(current_joint_));
        memcpy(qd, current_vel_joint_, sizeof(current_vel_joint_));
        mutex_.unlock();
    }
        ...
        while (is_running_)
        {
            if (real_time_tcp_->isConnect())
            {
                ...
                try
                {
                    if (real_time_tcp_->tcpRecv(&real_time_data_, sizeof(real_time_data_), has_read, 5000))
                    {
                        ...
                        for (uint32_t i = 0; i < 6; i++) {
                            ...
                            current_vel_joint_[i] = deg2Rad(real_time_data_.qd_actual[i]);
                ...
                catch (const TcpClientException& err)
                {
                    ...
                    RCLCPP_ERROR(getLogger(), "TCP recv error: %s", err.what());
            ...
            if (!motion_port_tcp_->isConnect())
            {
                RCLCPP_ERROR(getLogger(), "hello");
                try
                {
                    motion_port_tcp_->connect();
                }
                catch(const TcpClientException& err)
                {
                    RCLCPP_ERROR(getLogger(), "TCP recv error: %s", err.what());
    ...
    void init()
    {
        ...
        {
            RCLCPP_ERROR(getLogger(), "Commander: %s", err.what());
    ...
    {
        return dash_board_tcp_->isConnect() && real_time_tcp_->isConnect() && motion_port_tcp_->isConnect();
    }
    ...
    void movJ(double x, double y, double z, double a, double b, double c)
    {
        ...
        motion_port_tcp_->tcpSend(cmd, strlen(cmd));
    }
    ...
    rclcpp::Logger getLogger() {
        return rclcpp::get_logger("CR16_commander");
    }
